    ...
    .object({
        ...
        server: z
            .object({
                ...
                reload: z
                    .object({
                        secret: configuredSecretSchema('server.reload.secret'),
                    })
                    .strict()
                    .optional(),
                securityState: z
...
export type MagicSsoTomlHostedAuth = z.infer<typeof rawHostedAuthSchema>;
export type MagicSsoTomlSiteAccessRule = z.infer<typeof rawSiteAccessRuleSchema>;
export type MagicSsoTomlSite = z.infer<typeof rawSiteSchema>;
export type MagicSsoTomlConfig = z.infer<typeof rawConfigSchema>;
...
export function normaliseAllowedEmails(emails: string[], sourceName: string): string[] {
...
export function normaliseScopes(scopes: string[], sourceName: string): string[] {
...
export function buildSiteAccessRules(site: MagicSsoTomlSite): SiteAccessRules {
...
export interface RenderedSiteAccessRules {
    accessRules: MagicSsoTomlSiteAccessRule[];
    allowedEmails: string[];
}

export function renderSiteAccessRules(accessRules: SiteAccessRules): RenderedSiteAccessRules {
    const allowedEmails: string[] = [];
    const scopedAccessRules: MagicSsoTomlSiteAccessRule[] = [];
    const sortedEmails = [...accessRules.keys()].sort((left, right) => left.localeCompare(right));

    for (const email of sortedEmails) {
        const scopes = accessRules.get(email);
        ...
        if (scopes.has(FULL_ACCESS_SCOPE)) {
            if (scopes.size > 1) {
                throw new Error(
                    `Cannot render access rules for ${email} because full access and named scopes are mixed.`,
                );
            }
...
export function parseMagicSsoTomlConfig(
    fileContents: string,
    filePath: string,
): MagicSsoTomlConfig {
    ...
    if (!parsedConfig.success) {
        throw new Error(
            `Failed to validate MAGICSSO_CONFIG_FILE (${filePath}): ${formatTomlValidationIssue(parsedConfig.error.issues[0])}`,
        );
    }
...
export function stringifyMagicSsoTomlConfig(config: MagicSsoTomlConfig): string {
...
export function parseMagicSsoConfigToml(fileContents: string, filePath: string): AppConfig {
    ...
    const hostedAuthBranding = resolveHostedAuthBranding(
        parsedConfig.hostedAuth.branding,
    ...
    return {
        ...
        emailSmtpPass: parsedConfig.email.smtp.pass,
        emailSmtpPort: parsedConfig.email.smtp.port,
        emailSmtpSecure: parsedConfig.email.smtp.secure,
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
    ...
    const config = parseMagicSsoConfigToml(fileContents, filePath);
