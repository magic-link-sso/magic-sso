...
<html lang="en">
  <head>
    ...
    <title>Confirm Sign In | Magic Link SSO Photos Demo</title>
    <style>
      * {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
    ...
    if (
        ...
        const response = buildLoginRedirect(appOrigin, returnUrl, 'verify-email-failed');
    ...
    if (typeof serverUrl !== 'string' || serverUrl.length === 0) {
        console.error('Verify-email route is missing MAGICSSO_SERVER_URL.');
        const response = buildLoginRedirect(appOrigin, returnUrl, 'verify-email-misconfigured');
        clearVerifyCookie(response, request);
        return response;
    }
    ...
    try {
        ...
        const redirectUrl = new URL(returnUrl, appOrigin);
        ...
        const redirectResponse = buildLoginRedirect(appOrigin, returnUrl, 'verify-email-failed');
        clearVerifyCookie(redirectResponse, request);
        return redirectResponse;
    }
}
